...
class EnvelopeLinkObjectTest {
    ...
    void testGetCsvString() {
        ...
        EnvelopeLinkObject obj = new EnvelopeLinkObject();
        ...
        obj.setSize(1);
        obj.setTimeToLive(LocalDateTime.now());
        ...
        assertEquals(String.valueOf(obj.getContainerType().getValue()), csv[0]);
        assertEquals(obj.getDataProductType().name(), csv[1]);
        assertEquals(obj.getExchangeMetadata().getDataProtection().toString(), csv[2]);
        assertEquals(obj.getExchangeMetadata().getProtectionScheme(), csv[3]);
        assertEquals(obj.getExchangeMetadata().getDigitalSignatureReference().toString().toLowerCase(), csv[4]);
        assertEquals(obj.getExchangeMetadata().getDigitalSignatureValue().getPublicRootCertificateThumbprint(), csv[5]);
        assertEquals(obj.getExchangeMetadata().getDigitalSignatureValue().getPublicCertificate(), csv[6]);
        assertEquals(obj.getExchangeMetadata().getDigitalSignatureValue().getDigitalSignature(), csv[7]);
        assertEquals(obj.getExchangeMetadata().getCompressionFlag().toString(), csv[8]);
        assertEquals(obj.getFromSubscription().toString(), csv[9]);
        assertEquals(String.valueOf(obj.getAckRequest().getValue()), csv[10]);
        assertEquals(obj.getTransactionIdentifier().toString(), csv[11]);
        assertEquals(obj.getEnvelopeSignatureCertificate(), csv[12]);
        assertEquals(obj.getEnvelopeRootCertificateThumbprint(), csv[13]);
        assertEquals(String.valueOf(obj.getSize()), csv[14]);
        assertEquals(String.valueOf(obj.getTimeToLive().toEpochSecond(ZoneOffset.UTC)), csv[15]);
        assertEquals(String.valueOf(obj.getEnvelopeSignatureTime().toEpochSecond(ZoneOffset.UTC)), csv[16]);
    }
